...
export default class playlistCard extends Component {
  ...
  cardHeader = (img_name) => (
    <img className="play-img" alt="Card" src={img_name} />
  );
  ...
  getSongsForPlayListCode() {
    ...
    var req = new Request(
      'http://localhost:4000/playlist/api/getsongsforcode',
      {
        method: 'POST',
        headers: new Headers({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({
          user: this.props.fwduser,
  ...
  render() {
    var img_path;
    if (this.props.playlist.genre === 'bluesMusic') img_path = img_blue;
    else if (this.props.playlist.genre === 'rockMusic') img_path = img_rock;
    else if (this.props.playlist.genre === 'rocknrollMusic')
      img_path = img_rockroll;
    else if (this.props.playlist.genre === 'soulMusic') img_path = img_rock;
    else if (this.props.playlist.genre === 'hiphopMusic') img_path = img_hip;
      ...
      >
        ...
        >
          {this.props.playlist.collaborators.length > 0 && (
            <div>Collaborators: {this.props.playlist.collaborators}</div>
          )}
        </Card>
